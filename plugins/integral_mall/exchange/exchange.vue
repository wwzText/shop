<template>
    <app-layout>
        ...
        <view class='coupon' v-if="activeTab == 1" v-for="item in list" :key="item.id">
            ...
            <view class='coupon-price' :class="getTheme+'-m-text ' + getTheme">{{item.integralCoupon.integral_num}}积分
                <text v-if="item.integralCoupon.price > 0">+{{item.integralCoupon.price}}元</text>
            </view>
            <view @click="toList" class="card-list">
                <button class='to-card' :class="getTheme + '-m-text ' + getTheme">去卡券包查看</button>
            </view>
        </view>
        <view class='goods' v-if="activeTab == 2" v-for="item in list" :key="item.id">
            ...
            <view class='goods-info'>
                ...
                <view class='coupon-price' :class="getTheme+'-m-text ' + getTheme">{{item.detail[0].goods_info.goods_attr.extra.integral_num}}积分+{{item.detail[0].goods_info.goods_attr.price}}元</view>
            </view>
            <view @click="toOrder(item.id)" class="order-detail">
                <button class='to-card' :class="getTheme + '-m-text ' + getTheme">订单详情</button>
            </view>
        </view>
    </app-layout>
</template>
