    ...
    <app-layout :overflow='false'>
		<view v-if="customize">
	        ...
	        <view @click="changeSwitch" class='tips dir-left-nowrap cross-center' v-if="config.is_remind == 1">
	            ...
	            <switch style="transform:scale(0.7)" v-if="user.is_remind == 1" @change='toSwitch' :checked="user.is_remind"></switch>
	            <switch style="transform:scale(0.7)" v-else @change='toSwitch' :checked="user.is_remind"></switch>
	        </view>
	        <!-- 规则 -->
	        ...
	        <view class='other' :style="{'background':`linear-gradient(to bottom,${customize.end_bg},${customize.end_style == 1 ? customize.end_gradient_bg : customize.end_bg})`,'min-height':`${other_height}`}">
            	...
            	<!-- #endif -->
	                ...
	                <view class='everyday main-between'>
	                    ...
	                    <!-- #endif -->
	                        <image load-lazy='true' v-if="item.check" :src="customize.calendar_icon ? customize.calendar_icon : './../image/choose.png'"></image>
	                        <view :class="[`${item.date > now ? 'next' : ''}`]">{{item.date}}</view>
	                    </view>
	                </view>
	            </view>
	        </view>
