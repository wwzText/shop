<template>
    <app-layout>
        ...
        <view class="info">
            ...
            <image v-if="detail.level_up_log.status == 1" class="success-img" :src="bonus.success"></image>
            <view v-if="detail.level_up_log.status == 1" class="update main-between cross-center">
            ...
            <view v-if="detail.is_up == 1" class="update main-between cross-center">
                <view v-if="detail.level_up_log.status == 0">正在申请成为{{detail.province_text}}的{{detail.level_up.level_desc}}</view>
                <view v-else>可升级为{{detail.province_text}}的{{detail.level_up.level_desc}}</view>
                <view @click="toUpdate" class="update-btn">{{detail.level_up_log.status == 0 ? '去查看':'去升级'}}</view>
            </view>
            ...
            <view class="relation-list" v-if="detail.level > 1 && !(detail.relation.length == 1 && detail.relation[0].name == detail.province_text)">
                <text v-for="(item,index) in detail.relation" :key="inde">
