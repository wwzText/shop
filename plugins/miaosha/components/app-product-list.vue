			...
			<view class="app-content">
				...
				<view class="price-button dir-left-nowrap main-between cross-bottom">
					<view class="price app-price dir-top-nowrap main-right cross-top">
						...
						<view class="app-min-price" :class="[theme + '-m-text', theme]">￥{{item.goods.min_price}}</view>
						<view class="app-original-price">￥{{item.goods.original_price}}</view>
					</view>
					<view class="app-button" style="color: #ffffff;" :class="[theme + '-m-back', theme]" v-if="status === 1 && item.miaosha_num > 0">马上抢购</view>
					<view class="app-button no-button" :class="[theme + '-m-text', theme + '-m-border',theme]" v-if="(status === 0 || status === 2) && item.miaosha_num > 0">查看商品</view>
