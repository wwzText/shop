<template>
    <app-layout>
        ...
        <view>
            <view class="order-item" v-for="item in list" :key="item.id">
                ...
                <view class="dir-right-nowrap menu-button">
                    ...
                    <view @click.stop="toExpress(item,2)" v-if="item.have_refund != 1 && item.cancel_status == 0 && item.is_send == 1 && item.is_confirm == 0 && item.detailExpress.length > 0" class="handle-btn other">修改物流信息</view>
                    ...
                    <view @click.stop="toRefund(item)" v-if="item.status == 2 && item.status_text == '待退款' && item.is_confirm == 1 && status == 2 && item.type == 1" class="handle-btn other">退款</view>
        ...
        <app-time-screening theme="a" @click="toChoose" @cancel="cancel" color="#ff4544" v-if="chooseTime" :time="time" :start-date="date_start" :end-date="date_end"></app-time-screening>
