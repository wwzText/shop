<template>
    <app-layout>
        ...
        <view class="user-info" v-if="(order.address || order.send_type != 2) &&  (order.goods_type === 'goods' || order.goods_type === 'exchange')" :style="{padding: order.name || order.mobile ? '28rpx 24rpx 0' : '44rpx 24rpx 20rpx' }">
            ...
            <view v-if="order.name || order.mobile" class='main-center area'>
        ...
        <view class="express" v-if="order.detailExpress.length > 1 || (order.is_send == 0 && order.detailExpress.length >= 1)" @click="toExpressMore(order)">
            <view class="showMore">该订单已拆成多个包裹发货，点击查看详情</view>
            <image class="to-more" src='/static/image/icon/arrow-right.png'></image>
        </view>
        <view class="express" v-else-if="detail.detailExpress.length > 1 || (detail.is_send == 0 && detail.detailExpress.length >= 1)" @click="toExpressMore(detail)">
            <view class="showMore">该订单已拆成多个包裹发货，点击查看详情</view>
            <image class="to-more" src='/static/image/icon/arrow-right.png'></image>
        </view>
        ...
        <view :class="[`${!order.address && order.send_type != 1 ? 'goods-info card-info':'goods-info'}`]" :style="{marginTop: order.goods_type === 'goods' && order.address == '' && order.send_type == 2? '140rpx' : '24rpx'}">
            <view v-if="order.sign !== 'composition' || status == 2">
                ...
                <view v-for="(goods, dIndex) in order.detail" class="goods" :key="goods.id">
                    ...
                    <view class="goods-price">
                        <span v-if="order.plugin_data.exchange_list && order.plugin_data.exchange_list.length">{{order.plugin_data.exchange_list[dIndex].value}}{{order.plugin_data.price_name}}+</span>
                        <span>￥{{goods.total_original_price}}</span>
                    </view>
                </view>
            </view>
            <view v-if="order.plugin_data.exchange_count == null" class="main-between price-item">
                <view class="price-label">商品总价</view>
                <view>￥{{ order.total_goods_original_price }}</view>
            </view>
            <view v-if="order.plugin_data.exchange_count" class="main-between price-item">
                <view class="price-label">兑换{{order.plugin_data.price_name}}</view>
                <view>{{ order.plugin_data.exchange_count }}</view>
            </view>
            <view v-if="order.plugin_data.exchange_count >= 0" class="main-between price-item">
            ...
            <view v-if="order.plugin_data.discount_list" class="main-between price-item" v-for="(pluginData,index) in order.plugin_data.discount_list" :key="index">
            ...
            <view class="main-between price-item" style='margin-bottom: 0'>
                ...
                <view class="total-price">
                    <text v-if="order.plugin_data.exchange_count">{{order.plugin_data.exchange_count}}{{order.plugin_data.price_name}}+</text>
        ...
        <view class='goods-info mch-goods' v-if="order.sign !== 'vip_card'">
            ...
            <template v-if="order.send_type == 2">
                ...
                <view class="sc" v-if="(order.detailExpress.length == 1 || order.city_name) && order.is_send == 1">
        ...
        <view class='goods-info' v-if="showForm&& status == 1">
            ...
            <view v-for="(row,idx) in formList" :class="['goods-form', `${idx == 0? 'more':''}`]" :key="row.id">
                <view v-for="(goods,index) in row" :key="index">
        ...
        <view :class="['btn', `${iphone_x ? 'iphone_x':''}`]" v-if="(order.is_confirm != 1 && order.cancel_status == 0) || status == 2">
            <order-menu :hiddenRemark="hiddenRemark" @change="cancel" :address="address" :status="status" @update="update" :item="status == 2 ? detail : order"></order-menu>
        </view>
        <order-menu v-else :hiddenRemark="hiddenRemark" @change="cancel" :address="address" :status="status" @update="update" :item="status == 2 ? detail : order"></order-menu>
    </app-layout>
</template>
