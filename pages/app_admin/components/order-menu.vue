<template>
	<view>
	    <view class="dir-right-nowrap">
	        ...
	        <view @click.stop="refundHandle(2,item,1)" v-if="item.action_status.is_show_cancel_refund" :class="['handle-btn', `${_num == 1 || _num == -1?'other':''}`]">取消售后</view>
	        <!-- 换货发货 -->
	        ...
	        <view v-if="item.cancel_status == 0 && item.is_send == 1 && item.is_confirm == 0 && item.express_no" @click.stop="toExpress(item,2)" class="handle-btn other">修改物流信息</view>
	        <view @click="toExpressInfo(item)" v-if="item.cancel_status == 0 && item.is_send == 1 && item.is_confirm == 0 && item.detailExpress.length > 0" class="handle-btn other">查看物流</view>
	        ...
	        <view @click="toExpressInfo(item)" v-if="status == 2 && item.detailExpress.length > 0 && item.status == 2" class="handle-btn">物流详情</view>
        ...
        <view class="bg cross-center" v-if="isRefund || noRefund || changePrice || cancelOrder || item.showMobile || notRefund || isReason || openAddress || noAddress || beRemark || confirmOrder || refundOnly || cityRefund || beToSend">
            ...
            <view class="dialog" v-if="item.showMobile">
                <view class="main-center cancel-tip">{{item.mobile}}</view>
                <view class="main-center btn-area">
                    <view class="submit-btn" @click='cancel'>取消</view>
                    <view class="line"></view>
            ...
            <!-- 理由 -->
            <view class="dialog get-top" v-if="isReason">
                ...
                <view class="reason">
                    <template v-if="detail.refund_data.cause || detail.cancel_data.cause">
                    ...
                    <template v-if="(status == 2 &&detail.remark) || detail.cancel_data.remark">
                        ...
                        <view v-if="(status == 2 &&detail.remark.length > 40) || detail.cancel_data.remark.length > 40" class="main-center cross-center show-remark">
                    ...
                    <view v-if="detail.cancel_data.image_list.length > 0" class="reason-title">图片凭证</view>
                    <view class="dir-left-wrap reason-img" v-if="detail.cancel_data.image_list.length > 0">
