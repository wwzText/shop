<template>
    <app-layout>
        <scroll-view @scrolltolower="getMore" scroll-y :style="{'height':`${windowHeight}px`}">
            ...
            <view v-if="!search && status==1" class="main-between navs-info">
                <view @click="tab(-1)" class="box-grow-1 main-center cross-center navs">
                    <text :class="[`${_num==-1?'active':'nav-text'}`]">全部</text>
                </view>
                <view @click="tab(0)" class="box-grow-1 main-center cross-center navs">
                    <text :class="[`${_num==0?'active':'nav-text'}`]">待付款</text>
                </view>
                <view @click="tab(1)" class="box-grow-1 main-center cross-center navs">
                    <text :class="[`${_num==1?'active':'nav-text'}`]">待发货</text>
                </view>
            ...
            <view v-if="!search && status==2" class="main-between navs-info">
                ...
                <view @click="tab(0)" class="box-grow-1 main-center cross-center navs">
                    <text :class="[`${_num==0?'active':'nav-text'}`]">待审核</text>
                </view>
                <view @click="tab(1)" class="box-grow-1 main-center cross-center navs">
                    <text :class="[`${_num==1?'active':'nav-text'}`]">待买家处理</text>
                </view>
                <view @click="tab(2)" class="box-grow-1 main-center cross-center navs">
                    <text :class="[`${_num==2?'active':'nav-text'}`]">待卖家处理</text>
                </view>
            </view>
            <!-- 更多隐藏tab栏 -->
                ...
                <view class="order-item" v-for="item in list" :key="item.id">
                    <view @click="toDetail(item.id,status)">
                        ...
                        <view class="dir-left-nowrap cross-center order-user" v-if="status ==1">
                            <view>{{item.name ? item.name : item.nickname}}</view>
                        ...
                        <view class="dir-left-nowrap cross-center order-user" v-if="status ==2">
                            ...
                            <view>{{item.order.mobile}}</view>
                            <view v-if="item.order.mobile" @click.stop='toCall(item.order)' class="to-tel dir-left-nowrap cross-center">
                                <image src='./../image/tel.png'></image>
                                <text>联系收货人</text>
                            </view>
                        </view>
                        <view v-for="(goods, dIndex) in item.detail" class="goods" :key="goods.id">
                            ...
                            <view class="goods-price">
                                <span v-if="item.plugin_data.exchange_list && item.plugin_data.exchange_list.length">{{item.plugin_data.exchange_list[dIndex].value}}{{item.plugin_data.price_name}}+</span>
                                ￥{{goods.total_price}}
                            </view>
                        </view>
                    ...
                    <view class="menu-button">
                        <order-menu @change="cancel" :address="address" @to="toRefundOrder" :status="status" @update="update" :item="item"></order-menu>
                    </view>
                </view>
            </view>
        ...
        <app-time-screening theme="admin" @click="toChoose" @cancel="cancel" v-if="chooseTime" :time="time" :start-date="date_start" :end-date="date_end"></app-time-screening>
    </app-layout>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 搜索
            searchMethod() {
                let value = uni.getStorageSync('keyword');
