    ...
    <view class="modal-box" v-if="isShow" @click="confirm('close')">
        <view class="modal-item" @click.stop.prevent="">
            ...
            <view class="picker-box">
                <picker-view indicator-class="picker-item" :value="value" :indicator-style="indicatorStyle" :style="{'height': height}" @change="change">
                    <picker-view-column style="text-align: center;">
                        <view :class="{'line': newValue == index, 'line-2': newValue != index}" v-for="(item, index) in list" :key="index">{{item}}</view>
                    </picker-view-column>
                </picker-view>
            </view>
        </view>
