<template>
    <view class="app-shop-product">
        ...
        <view v-for="(good, index) in mch.goods_list" :key="index">
            <view class="app-product dir-left-nowrap cross-top" :class="{'no-border': index == mch.goods_list.length - 1}">
                <view class="app-single-radio dir-left-nowrap main-center cross-center">
                    ...
                    <view v-else-if="(good.new_status === 6 || mch.new_status === 2) && !editStatus" class="app-radio dir-top-nowrap main-center cross-center">
                ...
                <view class="app-content" :style="{'margin-bottom': good.sign === 'wholesale' ? good.new_status == 0 || good.new_status == 6 ?'0' : '50rpx' : '30rpx'}">
                    <view class="app-wholesale-title" v-if="good.sign === 'wholesale'" :class="{'app-failed-title': good.new_status !== 0}">
                    ...
                    <text class="app-title" v-else :class="{'app-failed-title': good.new_status !== 0}">
                        {{good.goods.name}}
                    </text>
                    ...
                    <view class="app-miaosha dir-left-nowrap main-left cross-center" v-if="good.sign === 'flash_sale'">
                        ...
                        <icon class="app-flash-sale-spike-icon" v-if="good.flash_sale_status === 2 || good.flash_sale_status === 0"></icon>
            ...
            <view class="app-wholesale-style" :class="{'border': good.sign != 'wholesale'}" v-if="mch.mch_id == 0 && ((good.sign != 'wholesale' && mch.goods_list[index+1].sign === 'wholesale') || (good.sign === 'wholesale' && (good.new_status === 0 || good.new_status === 6)))">
                <view class="app-wholesale-attr dir-left-nowrap cross-center" v-for="(attr, idx) in good.attr_arr" :key="idx">
                    <view class="app-single-radio dir-left-nowrap main-center cross-center">
                        <view v-if="(good.new_status === 6 || mch.new_status === 2) && !editStatus" class="app-radio dir-top-nowrap main-center cross-center">
                            <view class="disabled-radio"></view>
                        </view>
                        <view class="app-radio dir-top-nowrap main-center cross-center" v-else
                    ...
                    <view class="attr-wholesale-item main-between cross-center">
                        <view class="attr-item-name">
                            ...
                            <view :class="theme+ '-m-text ' + theme">￥{{attr.price ? attr.price : attr.attrs.price}}</view>
                        </view>
                        <app-add-subtract
                            v-model="attr.num"
                            :good_id="attr.id"
                ...
                <view class="wholesale-total dir-right-nowrap cross-center" v-if="good.sign === 'wholesale' && (good.new_status === 0 || good.new_status === 6)">
                    <view class="wholesale-total-price" :class="theme+ '-m-text ' + theme">￥{{good.price}}</view>
                    <view>共{{good.attrs.num}}{{good.goods.goodsWarehouse.unit}}</view>
                </view>
            </view>
        </view>
    </view>
